...
def _train_lstm_base(X_tr, y_tr, X_va, device, args):
    ...
    model = LSTMModel(feature_dim=X_tr.shape[-1], hidden_dim=args.lstm_hidden_dim, num_layers=1, dropout=0.0, mode='cls_bottom').to(device)
...
def train(args):
    ...
    if 'lstm' in args.models:
        base_probs['lstm'] = _train_lstm_base(X_tr, y_tr, X_va, device, args)
    if 'tf' in args.models:
        base_probs['tf'] = _train_tf_base(X_tr, y_tr, X_va, device, args)
    if 'xgb' in args.models:
        base_probs['xgb'] = _train_xgb_base(X_tr, y_tr, X_va, args)

    # Stack meta: logistic regression on base probabilities
    keys: List[str] = sorted(base_probs.keys())
    X_meta = np.stack([base_probs[k] for k in keys], axis=1)
    ...
    report = compute_classification_report(y_va, meta_prob)
    from backend.app.training.utils import precision_recall_at_k as _prk
    prk_metrics = _prk(y_va, meta_prob)
    # Regime: low volatility on validation sequences
    metrics_lowvol = {}
    prk_lowvol = {}
    try:
        from backend.app.training.utils import volatility_mask
        mask = volatility_mask(X_va, percentile=getattr(args, 'regime_percentile', 0.5), feature_index=0) if getattr(args, 'regime_filter', 'none') == 'low_vol' else None
        if mask is not None and mask.shape[0] == len(y_va) and mask.any():
            report_low = compute_classification_report(y_va[mask], meta_prob[mask])
            metrics_lowvol.update(report_low)
            prk_lowvol = _prk(y_va[mask], meta_prob[mask])
    except Exception as e:
    ...
    high_tp = int(((pred_high == 1) & (y_va == 1)).sum()) if high_pos > 0 else 0
    low_tp = int(((pred_low == 1) & (y_va == 1)).sum()) if low_pos > 0 else 0
    ...
    payload = {
        ...
        'split': {'n_total': int(n), 'n_train': int(split), 'n_val': int(n - split)},
    }
...
def main():
    ...
    p.add_argument('--ensemble', type=str, default='logistic', choices=['logistic','dynamic'], help='Meta ensemble method')
