...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        try:
            pe_slice = self.pe[:, :x.size(1)]  # type: ignore[index]
        except Exception:
            pe_slice = 0.0
        x = x + pe_slice
...
class TransformerModel(nn.Module):
    """Pre-LN Transformer encoder with optional stochastic depth and pooled last token head."""
    def __init__(self, feature_dim: int, model_dim: int, nhead: int, num_layers: int, dropout: float, mode: str, pos_dropout: float = 0.0):
        super().__init__()
        self.mode = mode
        self.in_proj = nn.Linear(feature_dim, model_dim)
        self.pos_enc = PositionalEncoding(model_dim, dropout=pos_dropout)
        self.layers = nn.ModuleList([
            nn.TransformerEncoderLayer(d_model=model_dim, nhead=nhead, dim_feedforward=model_dim*4, dropout=dropout, batch_first=True, activation='gelu')
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            # Pre-LN style: apply norm before layer for stability
...
def train(args):
    ...
    if getattr(args, 'fine_tune', False):
        ...
        log.info(f"[TF][fine-tune] enabled days={effective_days} (original {args.days})")

    if args.mode == 'reg_next_ret':
        X, y = build_sequence_dataset(days=effective_days, seq_len=args.seq_len, interval=args.interval, feature_subset=feature_subset, data_source=getattr(args, 'data_source', 'synthetic'))
    else:
        try:
    ...
    # Time-ordered split with optional test holdout
    ...
    model = TransformerModel(feature_dim=X.shape[-1], model_dim=args.model_dim, nhead=args.nhead, num_layers=args.num_layers, dropout=args.dropout, mode=args.mode, pos_dropout=args.pos_dropout).to(device)
    # Load from checkpoint for fine-tune
    if getattr(args, 'fine_tune', False):
        load_candidates = []
        ...
        if out_dir:
            load_candidates.append(os.path.join(out_dir, 'last_tf.pt'))
        loaded = False
        for pth in load_candidates:
            try:
                if pth and os.path.exists(pth):
                    ...
                    log.info(f"[TF][fine-tune] loaded checkpoint from {pth} missing={len(missing)} unexpected={len(unexpected)}")
                    loaded = True
                    break
            except Exception as e:
                log.warning(f"[TF][fine-tune] failed to load {pth}: {e}")
        if not loaded:
            log.warning("[TF][fine-tune] no checkpoint loaded; training from scratch on recent window")
    ...
    opt = torch.optim.AdamW(model.parameters(), lr=lr_use, weight_decay=args.weight_decay)
    scheduler = None
    if args.scheduler.lower() == 'cosine':
        # Cosine with warmup
        warmup = max(1, int(args.warmup_epochs))
        total = args.epochs
        def _lr_lambda(ep):
            if ep < warmup:
                return (ep + 1) / warmup
    ...
    if args.mode == 'cls_bottom':
        ...
        if args.focal_gamma > 0:
            # focal loss wrapper
            bce = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_weight_value], dtype=torch.float32, device=device), reduction='none')
            def focal_loss(logits, targets):
                targets_f = targets.float()
                bce_loss = bce(logits, targets_f)
                p = torch.sigmoid(logits)
                pt = p * targets_f + (1 - p) * (1 - targets_f)
                gamma = args.focal_gamma
                loss = ((1 - pt) ** gamma) * bce_loss
                return loss.mean()
            loss_fn = focal_loss
            log.info(f"[TF][cls] focal loss enabled gamma={args.focal_gamma} pos_weight={pos_weight_value:.4f}")
        else:
    ...
    # Final test evaluation (if present)
    test_metrics = {}
    if dl_te is not None:
        ...
        if val_logits:
            ...
            if args.mode == 'cls_bottom':
                ...
                log.info("[TF][cls][test] P=%.4f R=%.4f F1=%.4f AUROC=%s AUPRC=%s",
                ...
                log.info("[TF][reg][test] RMSE=%.6f MAE=%.6f DirAcc=%.3f", rmse, mae, dir_acc)
    if test_metrics:
        try:
...
def main():
    ...
    p.add_argument('--pos-dropout', type=float, default=0.05)
    p.add_argument('--lr', type=float, default=3e-4)
    p.add_argument('--weight-decay', type=float, default=0.01)
    p.add_argument('--warmup-epochs', type=int, default=2)
    p.add_argument('--epochs', type=int, default=15)
    p.add_argument('--focal-gamma', type=float, default=0.0, help='>0 to enable focal loss (e.g. 2.0)')
    ...
    p.add_argument('--ft-lr', type=float, default=float(os.getenv('SEQ_FT_LR', '3e-4')))
    p.add_argument('--load-from', type=str, default=os.getenv('SEQ_FT_LOAD_FROM', ''), help='Optional checkpoint path; defaults to model-out or last_tf.pt')
    args = p.parse_args()
    train(args)
