    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for xb, yb in dl_va:
                ...
                val_preds.append(p.cpu().numpy()); val_targets.append(yb.numpy())
        if val_preds:
            vp = np.concatenate(val_preds); vt = np.concatenate(val_targets)
            if args.mode == 'cls_bottom':
                ...
                log.info(f"[TF][cls] Epoch {epoch}/{args.epochs} train_loss={np.mean(train_losses):.6f} TP={tp} FP={fp} FN={fn} P={precision:.4f} R={recall:.4f} F1={f1:.4f}")
            else:
                rmse = float(np.sqrt(np.mean((vp - vt)**2)))
                mae = float(np.mean(np.abs(vp - vt)))
                dir_acc = float(np.mean(np.sign(vp) == np.sign(vt)))
                log.info(f"[TF][reg] Epoch {epoch}/{args.epochs} train_loss={np.mean(train_losses):.6f} rmse={rmse:.6f} mae={mae:.6f} dir_acc={dir_acc:.3f}")
        else:
            log.info(f"[TF] Epoch {epoch}/{args.epochs} train_loss={np.mean(train_losses):.6f} (no val samples)")
