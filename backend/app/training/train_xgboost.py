...
def train(days: int, val_ratio: float, max_depth: int, n_estimators: int, learning_rate: float, model_out: str, mode: str, interval: str,
    ...
    if tb:
        if SummaryWriter is None:
            ...
            run_name = f"xgb_{mode}_{interval}_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    if mode == 'reg_next_ret':
        ...
        metrics.update({'rmse': rmse, 'mae': mae, 'direction_acc': direction_acc, 'best_iteration': booster.best_iteration})
        log.info("[XGB][reg] RMSE=%.6f MAE=%.6f DirAcc=%.3f", rmse, mae, direction_acc)
        if writer is not None:
            try:
                writer.add_scalar('val/rmse', rmse, booster.best_iteration if hasattr(booster, 'best_iteration') else 0)
                writer.add_scalar('val/mae', mae, booster.best_iteration if hasattr(booster, 'best_iteration') else 0)
        ...
        metrics.update({'default_threshold': 0.5, 'precision': precision, 'recall': recall, 'f1': f1, 'tp': tp, 'fp': fp, 'fn': fn,
                        ...
                        'best_tp': best['tp'], 'best_fp': best['fp'], 'best_fn': best['fn'], 'best_iteration': booster.best_iteration})
        ...
        if writer is not None:
            try:
                writer.add_scalar('val/precision', precision, booster.best_iteration if hasattr(booster, 'best_iteration') else 0)
                writer.add_scalar('val/recall', recall, booster.best_iteration if hasattr(booster, 'best_iteration') else 0)
                writer.add_scalar('val/f1', f1, booster.best_iteration if hasattr(booster, 'best_iteration') else 0)
                writer.add_scalar('val/best_f1', best['f1'], booster.best_iteration if hasattr(booster, 'best_iteration') else 0)
    ...
    if writer is not None:
        try:
            # log params and metrics text
            writer.add_text('run/params', json.dumps(params, ensure_ascii=False), 0)
            writer.add_text('run/metrics', json.dumps(metrics, ensure_ascii=False), 0)
            writer.close()
        except Exception:
            pass
